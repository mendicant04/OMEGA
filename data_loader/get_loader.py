...
def get_loader(source_path, target_path, evaluation_path, transforms,
               batch_size=16, return_id=False, balanced=False, weights=None):
    ...
    return source_loader, target_loader, test_loader, target_folder_train, prototype_loader


def get_loader_class_inc(source_path, target_path, target_labeled_path, evaluation_path, transforms,
                         batch_size=16, return_id=False, balanced=False):
    source_folder = ImageFolder(os.path.join(source_path),
                                transforms['source'],
                                return_id=return_id)
    target_folder_train = ImageFolder(os.path.join(target_path),
...
def get_loader_balanced(source_path, target_path, evaluation_path, transforms, batch_size=16):
    source_folder = ImageFolder(os.path.join(source_path),
